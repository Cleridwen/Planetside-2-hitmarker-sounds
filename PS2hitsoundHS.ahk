...
SetWorkingDir %A_ScriptDir%  ; Ensures a consistent starting directory.
	...
	x1 :=  A_ScreenWidth //2 -18 ; defines the positions of pixels to scan
	y1 :=  A_ScreenHeight //2 -22
	x2 :=  A_ScreenWidth //2 +17
	y2 :=  A_ScreenHeight //2 +21


PixelExist(xPos, yPos) ; constantly checks if a pixel at a given location matches the right color
{
	CoordMode, Pixel, Screen
	PixelSearch, , , xPos, yPos, xPos, yPos, 0x7171ec, 20, Fast
	if ErrorLevel
		return 0
	else
		return 1
